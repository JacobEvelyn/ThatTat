jacobGlobals =
{
	imageData:
		[
			...
			title: "The Prestige",
			name: "Robert Angier",
			description: "That's why every magic trick has a third act, the hardest part. The part we call \"The Foot.\"",
			srcLo: "Prestige.jpg"},
			...
			title: "Jurassic Park",
			name: "Dr. Alan Grant",
			description: "Using sophisticated techniques, they extract the preserved blood from the mosquito, and bingo: dino feet.",
			srcLo: "Jurassic Park.jpg"},
			{archiveTitle: "Dragonslayer",
			title: "Dragonslayer",
			name: "Galen Bradwarden",
			description: "Unclean foot! Get thee down! Be thou consumed by the fires that made thee!",
...
function preloadImage(index)
{
	...
	if (index < 0)
		index = jacobGlobals.imageData.length - 1;
	else if (index > jacobGlobals.imageData.length - 1)
		index = 0;

	// if we've already loaded image, stop
	if (jacobGlobals.imageData[index].img != undefined)
		return;

	jacobGlobals.imageData[index].img = new Image();
	jacobGlobals.imageData[index].img.src = './Images/' + jacobGlobals.imageData[index].srcLo;
...
function initializeShare()
{
	...
	{
		window.open('http://twitter.com/home/?status=' + encodeURIComponent('That Tat - ' + jacobGlobals.imageData[parseInt(location.hash.substring(1)) - 1].archiveTitle) + ' ' + window.location.href);
		_gaq.push(['_trackSocial', 'Twitter', 'Tweet', window.location.href, window.location.pathname + window.location.search + window.location.hash]);
		return false;
	});

	document.getElementById('RedditButton').addEventListener('click', function()
	{
		window.open('http://reddit.com/submit/?url=' + encodeURIComponent(window.location.href));
		_gaq.push(['_trackSocial', 'Reddit', 'Submit', window.location.href, window.location.pathname + window.location.search + window.location.hash]);
		return false;
	});

	document.getElementById('StumbleUponButton').addEventListener('click', function()
	{
		window.open('http://stumbleupon.com/submit/?url=' + encodeURIComponent(window.location.href));
		_gaq.push(['_trackSocial', 'StumbleUpon', 'Submit', window.location.href, window.location.pathname + window.location.search + window.location.hash]);
		return false;
	});

	document.getElementById('PinterestButton').addEventListener('click', function()
	{
		window.open('http://pinterest.com/pin/create/button/?url=' + encodeURIComponent(window.location.href) + '&media=' + encodeURIComponent(document.getElementById('Container').firstChild.src));
		_gaq.push(['_trackSocial', 'Pinterest', 'Pin It', window.location.href, window.location.pathname + window.location.search + window.location.hash]);
		return false;
	});

	document.getElementById('GooglePlusButton').addEventListener('click', function()
	{
		window.open('https://plusone.google.com/_/+1/confirm?hl=en&url=' + encodeURIComponent(window.location.href));
		_gaq.push(['_trackSocial', 'Google+', '+1', window.location.href, window.location.pathname + window.location.search + window.location.hash]);
		return false;
	});

	document.getElementById('FacebookButton').addEventListener('click', function()
	{
		window.open('https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(window.location.href) + '&t=' + encodeURIComponent('That Tat - ' + jacobGlobals.imageData[parseInt(location.hash.substring(1)) - 1].archiveTitle));
		_gaq.push(['_trackSocial', 'Facebook', 'Share', window.location.href, window.location.pathname + window.location.search + window.location.hash]);
		return false;
	});
}
